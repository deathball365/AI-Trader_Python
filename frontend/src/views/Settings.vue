      ...
      <v-col cols="12">
        <v-card class="user-settings-card admin-service-card" elevation="0">
          ...
          <v-card-text>
            ...
            <div class="llm-section-head compact"><div><h3>{{ structureConfigScope === 'default' ? '公共结构配置' : '品种/周期结构配置' }}</h3><p>先配置结构识别和形态识别，再配置阶段/事件确认。phase 和 event 只在运行页动态显示。未覆盖字段自动继承公共默认值。</p></div><div class="d-flex ga-2 flex-wrap"><v-btn variant="tonal" color="secondary" :disabled="structureEngineSaving" @click="openSaveAsStructureProfile">另存为品种/周期配置</v-btn><v-btn color="primary" :loading="structureEngineSaving" @click="saveStructureEngineConfig">{{ structureConfigScope === 'default' ? '保存公共默认参数' : '保存当前品种/周期参数' }}</v-btn></div></div>
            ...
            <v-row class="mt-2">
              <v-col cols="12"><div class="structure-zone-heading"><div class="text-subtitle-2">一、结构识别 · Internal / Swing / External</div><div class="text-caption text-medium-emphasis">先决定系统“看见”哪些高低点。腿数越小越敏感，黄金 M1 会把每个小折返都当成结构；腿数越大越稳，适合看黄金 H1/H4 的大波段。这里不下单，只画结构和切段。</div></div></v-col>
              <v-col cols="12" sm="6" md="3"><v-text-field :class="structureFieldClass('pivot_legs')" v-model.number="structureEngineConfig.pivot_legs" type="number" min="2" max="12" label="小级别 Pivot 腿数" hint="Internal 用。数值越小，短线折返越多，入场更早也更容易被扫。黄金 M1 建议不要太小。" persistent-hint density="compact" variant="outlined" /></v-col>
              <v-col cols="12" sm="6" md="3"><v-text-field :class="structureFieldClass('medium_pivot_legs')" v-model.number="structureEngineConfig.medium_pivot_legs" type="number" min="3" max="30" label="中级别 Pivot 腿数" hint="Swing 用。决定主结构高低点。太大容易错过黄金亚洲盘那段单边，太小会把震荡当成趋势。" persistent-hint density="compact" variant="outlined" /></v-col>
              <v-col cols="12" sm="6" md="3"><v-text-field :class="structureFieldClass('large_pivot_legs')" v-model.number="structureEngineConfig.large_pivot_legs" type="number" min="5" max="60" label="大级别 Pivot 腿数" hint="External 用。看更大背景，用来过滤逆大方向的突破。" persistent-hint density="compact" variant="outlined" /></v-col>
              <v-col cols="12" sm="6" md="3"><v-text-field :class="structureFieldClass('min_reversal_atr')" v-model.number="structureEngineConfig.min_reversal_atr" type="number" min="0.1" max="5" step="0.1" label="最小反转幅度（ATR）" hint="高低点至少走出这么远才算有效拐点。调大可过滤黄金浅回撤造成的假结构。" persistent-hint density="compact" variant="outlined" /></v-col>
              ...
              <v-col cols="12"><div class="structure-zone-heading mt-2"><div class="text-subtitle-2">二、形态识别</div><div class="text-caption text-medium-emphasis">在当前结构段里判断价格是在箱体里磨、三角形里收口，还是沿着趋势线走。这里只回答“现在是什么形态”，不决定买卖。</div></div></v-col>
              <v-col cols="12"><div class="text-subtitle-2 mt-1">2.1 箱体</div><div class="text-caption text-medium-emphasis">解决“这段是不是在上下沿之间来回扫”的问题。例如黄金在 4270-4285 之间反复触碰，就可以判成箱体；如果把触碰次数或内部收盘比例设太高，浅震荡会被当成趋势。</div></v-col>
              ...
              <v-col cols="12" sm="6" md="3"><v-text-field :class="structureFieldClass('range_min_touches')" v-model.number="structureEngineConfig.range_min_touches" type="number" min="1" max="10" label="箱体最少触碰次数" hint="上下沿各至少碰几次才算箱体。太高会漏掉黄金短箱体。" persistent-hint density="compact" variant="outlined" /></v-col>
              ...
              <v-col cols="12" sm="6" md="3"><v-text-field :class="structureFieldClass('range_max_atr')" v-model.number="structureEngineConfig.range_max_atr" type="number" min="1" max="30" step="0.5" label="箱体最大宽度（ATR）" hint="太宽就不是箱体，而是波动区间。黄金大波动日可适当放宽。" persistent-hint density="compact" variant="outlined" /></v-col>
              <v-col cols="12" sm="6" md="3"><v-text-field :class="structureFieldClass('range_flat_angle_degrees')" v-model.number="structureEngineConfig.range_flat_angle_degrees" type="number" min="1" max="45" step="1" label="箱体水平角度（度）" hint="高点连线和低点连线相对水平线的夹角。默认 5°：小于它算水平箱体；两边都大于它并同向下倾，当成下降趋势，不再当箱体或三角形。" persistent-hint density="compact" variant="outlined" /></v-col>
              <v-col cols="12"><div class="text-subtitle-2 mt-2">2.2 三角形</div><div class="text-caption text-medium-emphasis">解决“波动是不是在收口”的问题。高点一波比一波低、低点一波比一波高，就是三角形。形态最少K线太短，容易把两三根回撤当成收敛。</div></v-col>
              <v-col cols="12" sm="6" md="3"><v-text-field :class="structureFieldClass('min_segment_bars')" v-model.number="structureEngineConfig.min_segment_bars" type="number" min="5" max="100" label="形态最少K线数" hint="三角形/收敛形态至少覆盖的结构段长度" persistent-hint density="compact" variant="outlined" /></v-col>
              <v-col cols="12"><div class="text-subtitle-2 mt-2">2.3 趋势线</div><div class="text-caption text-medium-emphasis">解决“是不是沿着一条斜线推进”的问题。黄金早盘抬高低点时，趋势线能画出上升通道；触碰太少或跨度太短，斜线会乱画。</div></v-col>
              ...
              <v-col cols="12" sm="6" md="3"><v-text-field :class="structureFieldClass('trendline_min_touches')" v-model.number="structureEngineConfig.trendline_min_touches" type="number" min="2" max="10" label="趋势线最少触碰次数" hint="至少几个点才能连成线。2 点就能画，但更容易被噪声带偏。" persistent-hint density="compact" variant="outlined" /></v-col>
              ...
              <v-col cols="12"><div class="structure-zone-heading mt-2"><div class="text-subtitle-2">三、阶段与事件确认</div><div class="text-caption text-medium-emphasis">形态有了以后，再判断现在走到哪一步：还在推进、已经成熟、还是出现了 BOS/CHoCH/回踩/回收。这些值由收盘K线自动算，这里只调“多严格才算数”。</div></div></v-col>
              <v-col cols="12"><div class="text-subtitle-2 mt-1">3.1 趋势阶段</div><div class="text-caption text-medium-emphasis">解决“还能不能追”的问题。刚突破后的黄金单边通常是推进；回撤变深后变成成熟，这时更适合等回踩，而不是继续追涨。</div></v-col>
              ...
              <v-col cols="12"><div class="text-subtitle-2 mt-2">3.2 BOS 延续突破</div><div class="text-caption text-medium-emphasis">解决“这波是不是顺着原方向再走一段”的问题。上涨里再创新高，就是 BOS。位移太小的浅突破，容易是假突破，所以要设最小位移和站稳根数。</div></v-col>
              <v-col cols="12" sm="6" md="3"><v-text-field :class="structureFieldClass('min_breakout_displacement_atr')" v-model.number="structureEngineConfig.min_breakout_displacement_atr" type="number" min="0.1" max="5" step="0.1" label="BOS 最小位移（ATR）" hint="默认 0.6；用于确认同向突破" persistent-hint density="compact" variant="outlined" /></v-col>
              <v-col cols="12" sm="6" md="3"><v-text-field :class="structureFieldClass('trend_max_event_age_bars_m1')" v-model.number="structureEngineConfig.trend_max_event_age_bars_m1" type="number" min="1" max="30" label="M1 BOS 观察窗口" hint="默认 5 根K线，超出后不再追踪该 BOS" persistent-hint density="compact" variant="outlined" /></v-col>
              <v-col cols="12" sm="6" md="3"><v-text-field :class="structureFieldClass('trend_max_event_age_bars_other')" v-model.number="structureEngineConfig.trend_max_event_age_bars_other" type="number" min="1" max="30" label="其他周期 BOS 观察窗口" hint="M5及以上默认 3 根K线" persistent-hint density="compact" variant="outlined" /></v-col>
              ...
              <v-col cols="12"><div class="text-subtitle-2 mt-2">3.3 CHoCH 结构转向</div><div class="text-caption text-medium-emphasis">解决“趋势是不是真的反了”的问题。上涨后收盘跌破前低，才切新结构段。确认根数太少，黄金一个长下影就会被当成反转。</div></v-col>
            ...
            <v-row class="mt-2">
              <v-col cols="12" sm="6" md="3"><v-text-field :class="structureFieldClass('entry_zone_atr')" v-model.number="structureEngineConfig.entry_zone_atr" type="number" min="0" max="3" step="0.05" label="入场区域（ATR）" hint="价格离计划价多近才算进场。太宽会在黄金快行情里追到很差的位置。" persistent-hint density="compact" variant="outlined" /></v-col>
            ...
            <v-row dense class="mt-3">
              ...
              <v-col cols="12">
                ...
                <div class="text-caption text-medium-emphasis mt-2">
                  有效 ATR 数据时，反转接近范围优先使用 ATR 倍数（默认 0.9）；没有足够 K 线时才回退到下单距离比例。止损为关键位 ±1，止盈按止盈比例计算。
                </div>
              </v-col>
            </v-row>
          </template>
...
export default {
  ...
  setup(props) {
    ...
    const structureFieldLabels = {
      ...
      range_min_inside_ratio: '箱体内部收盘比例', range_flat_angle_degrees: '箱体水平角度',
    ...
    const setupBindingDefaults = {
      range_breakout: { bind_pattern: 'range', bind_event: 'breakout_confirmed', direction_layer: 'swing', entry_layer: 'swing', require_external_alignment: false, entry_mode: 'breakout_retest', require_retest: true, require_reclaim: false, confirmation_bars: 2, min_displacement_atr: 0.6, min_body_atr: 0.5 },
      range_breakout_watch: { bind_pattern: 'range', bind_event: 'breakout_confirmed', direction_layer: 'swing', entry_layer: 'swing', require_external_alignment: false, entry_mode: 'close_breakout', require_retest: false, require_reclaim: false, confirmation_bars: 2, min_body_atr: 0.5 },
      range_false_breakout: { bind_pattern: 'range', bind_event: 'false_breakout', direction_layer: 'swing', entry_layer: 'internal', entry_mode: 'touch_and_reclaim', require_reclaim: true, require_retest: false, confirmation_bars: 2, false_breakout_require_reclaim_close: true, false_breakout_confirmation_bars: 2, false_breakout_min_reclaim_atr: 0.25, require_external_alignment: false },
      range_lower_reversal: { bind_pattern: 'range', bind_event: 'reclaim', direction_layer: 'swing', entry_layer: 'internal', require_external_alignment: false, entry_mode: 'touch_or_near', require_reclaim: true, require_retest: false },
      range_upper_reversal: { bind_pattern: 'range', bind_event: 'reclaim', direction_layer: 'swing', entry_layer: 'internal', require_external_alignment: false, entry_mode: 'touch_or_near', require_reclaim: true, require_retest: false },
      triangle_breakout: { bind_pattern: 'triangle', bind_event: 'breakout_confirmed', direction_layer: 'swing', entry_layer: 'swing', require_external_alignment: true, entry_mode: 'breakout_retest', require_retest: true, require_reclaim: false, confirmation_bars: 2, min_displacement_atr: 0.5, min_body_atr: 0.5 },
      triangle_breakout_watch: { bind_pattern: 'triangle', bind_event: 'breakout_confirmed', direction_layer: 'swing', entry_layer: 'swing', require_external_alignment: true, entry_mode: 'close_breakout', require_retest: false, require_reclaim: false, confirmation_bars: 2, min_body_atr: 0.5 },
      triangle_prebreakout_pullback: { bind_pattern: 'triangle', bind_event: 'retest', direction_layer: 'swing', entry_layer: 'internal', require_external_alignment: false, entry_mode: 'touch_or_near', require_retest: true, require_reclaim: false, retest_tolerance_atr: 0.35 },
      structure_location_pullback: { bind_pattern: 'trend', bind_event: 'retest', direction_layer: 'swing', entry_layer: 'internal', require_external_alignment: true, entry_mode: 'touch_and_reclaim', require_reclaim: true, require_retest: true, min_body_atr: 0.3, min_real_risk_reward: 0.5 },
      trend_continuation: { bind_pattern: 'trend', bind_event: 'bos', direction_layer: 'swing', entry_layer: 'internal', require_external_alignment: false, entry_mode: 'breakout_retest', require_retest: true, require_reclaim: false, confirmation_bars: 2, min_displacement_atr: 0.6, min_real_risk_reward: 0.5 },
      structure_reversal: { bind_pattern: 'trend', bind_event: 'choch', direction_layer: 'swing', entry_layer: 'swing', require_external_alignment: true, entry_mode: 'breakout_retest', require_retest: true, require_reclaim: false, confirmation_bars: 2, min_displacement_atr: 0.2 },
      choch_reversal: { bind_pattern: 'trend', bind_event: 'choch', direction_layer: 'swing', entry_layer: 'swing', require_external_alignment: true, entry_mode: 'breakout_retest', require_retest: true, require_reclaim: false, confirmation_bars: 2, min_displacement_atr: 0.2 },
      liquidity_sweep_reclaim: { bind_pattern: 'trend', bind_event: 'liquidity_sweep', direction_layer: 'swing', entry_layer: 'internal', require_external_alignment: false, entry_mode: 'touch_and_reclaim', require_reclaim: true, require_retest: false, min_displacement_atr: 0.3 },
    }
    ...
    const setupRuntimeDefaults = {
      ...
      bind_pattern: 'trend', bind_event: 'retest', direction_layer: 'swing', entry_layer: 'internal', require_external_alignment: true,
    ...
    const setupGuideTexts = {
      structure_location_pullback: 'Swing 向上只做多。买点是 HL：优先 Internal 自己的 HL；没有够近的 Internal HL，才用 Swing 的 HL。价格回到这个 HL 附近即可，一根 K 线砸到也行，只要没收盘跌破。默认还要求 Internal 仍是向上（浅回撤本来就不会翻向下）。若 Internal 已经跌破自己的 HL 翻成向下，原来的买点作废。若开着回收，要先刺破 HL 再收盘站回来。Internal 已经是箱体时，这个 SETUP 不出单。默认还要求 External 同向；大级别已经转空时，即使 Swing 还在涨也不买。',
      range_false_breakout: 'Swing 只定方向。箱体必须是 Internal 的上下沿，不会去用 Swing 的大箱体。下沿假突破：先跌破 Internal 下沿，再收盘回到箱内，买点就是这条下沿。上沿假突破同理卖上沿。Swing 上涨时只保留下沿那笔买。Internal 不是箱体时，这个 SETUP 不出。',
      range_lower_reversal: 'Swing 只定方向。买点只能是 Internal 箱体下沿。价格回到这条下沿附近买入。Internal 不是箱体，或者 Swing 在下跌，这个 SETUP 都不出。',
      range_upper_reversal: 'Swing 只定方向。卖点只能是 Internal 箱体上沿。价格回到这条上沿附近卖出。Internal 不是箱体，或者 Swing 在上涨，这个 SETUP 都不出。',
      ...
      triangle_breakout: '方向和入场都看 Swing 自己的三角形。收盘站上上沿后，买点是这条上沿，等回踩；收盘跌破下沿，卖点是下沿。实体太小或没越过边界，不算突破。默认要求 External 同向，避免逆大级别硬突破。',
      ...
      choch_reversal: '方向和入场都看 Swing。向上 CHoCH：买点是被打掉的那根 Swing 高点（常见是 LH），等回踩这根线。向下 CHoCH：卖点是被打掉的那根 Swing 低点（常见是 HL）。Internal 自己翻一下只当回撤，不会单独开反转单。默认要求 External 同向：大级别还没转时，把反向 CHOCH 当趋势内回撤，不当反转。',
    ...
    const selectStructureSetupScope = scope => {
      ...
      if (values.require_external_alignment === undefined || values.require_external_alignment === null) {
        values.require_external_alignment = builtin.require_external_alignment
      }
    ...
    const collectSetupDraft = () => {
      const draft = structureSetupProfileDraft.value
      const item = {}
      const always = ['enabled', 'allowed_directions', 'bind_pattern', 'bind_event', 'direction_layer', 'entry_layer', 'require_external_alignment']
      for (const key of setupFieldKeys) {
        ...
        else if (['enabled', 'require_reclaim', 'require_retest', 'invalidate_on_zone_return', 'false_breakout_require_reclaim_close', 'require_external_alignment'].includes(key)) item[key] = Boolean(value)
