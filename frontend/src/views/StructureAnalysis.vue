...
const phaseLabel=(value,bias)=>{if(value==='reversal_confirmed')return bias==='down'?'反转确认后的下跌延续':bias==='up'?'反转确认后的上涨延续':'反转已确认';return {forming:'形成中',continuation:'延续',pullback:'回撤中',reversal_candidate:'反转候选',breakout_confirmed:'突破已确认',mature:'成熟'}[value]||value||'--'}
const patternLabel=value=>({range:'箱体震荡',trend:'趋势',triangle:'三角形',converging_triangle:'收敛三角形',diverging_triangle:'扩散三角形',ascending_triangle:'上升三角形',descending_triangle:'下降三角形',trendline:'趋势线',none:'未形成'}[value]||value||'未识别')
