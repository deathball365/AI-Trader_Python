...
class TradingAccountRepository:
    ...
    def update_controls(
        ...
        *,
        ...
        if not 0.1 <= values["single_order_risk_limit"] <= 100:
            raise ValueError("单笔风险上限必须在 0.1% 到 100% 之间")
        now = _now_ts()
    ...
    @staticmethod
    def _row_to_account(row: Optional[Dict]) -> Optional[TradingAccountRecord]:
        if row is None:
            return None
